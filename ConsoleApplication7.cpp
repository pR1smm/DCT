...
int main()
{
	...
	*/
	...
	std::cout << "DCT2 Function Matrix" << std::endl;
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			std::cout << dct2TransformedMatrix.at(i).at(j) << ", ";
			//std::cout << matrixQuantisiert.at(i).at(j) << ", ";
		}
		std::cout << std::endl;
	}

	std::cout << std::endl;
	std::cout << "2*DCT Function Matrix" << std::endl;
	...
	std::vector<std::vector<float>> reversedTempMatrix(8, std::vector<float>(8));

	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
		{
			reversedTempMatrix.at(i).at(j) = tempMatrix.at(j).at(i);
			std::cout << reversedTempMatrix.at(i).at(j) << ", ";
		}
		std::cout << std::endl;
	}
	return 0;
